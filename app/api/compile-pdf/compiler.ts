...
async function handleCompileSuccess(
  ...
  if (pdfArrayBuffer.byteLength === 0) {
    throw new Error('bibby-compile returned empty response');
  }
...
async function handleCompileError(
  ...
  // Parse bibby-compile error response (always JSON on error)
  let errorData;
  try {
...
function handleCompileException(error: unknown): CompilerResponse {
  ...
  return {
    ...
    error: {
      ...
      suggestion: 'The bibby-compile service may be temporarily unavailable',
    },
  };
}
